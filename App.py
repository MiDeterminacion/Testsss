...
<style>
  body { margin:0; }
  #wrap { width:100%; height:80vh; background:#000; display:flex; align-items:center; justify-content:center; }
  canvas { display:block; width:100%; height:100%; }
  .hint { color:#aaa; font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif; font-size:14px; margin-top:.5rem; text-align:center;}
</style>
...
class Node {
  constructor(x, y, size) {
    this.x = x; this.y = y;
    this.size = size; // distancia del siguiente segmento
  }
}
...
class Worm {
  constructor(n, startX, startY) {
    this.nodes = [];
    this.end = { x: startX, y: startY }; // objetivo anterior
    this.speed = 6;                       // “holgura” al seguir
    // crea segmentos decrecientes
    let len = 18;
    for (let i = 0; i < n; i++) {
  ...
  // --- este moveTo está transcrito del snippet de la imagen ---
  moveTo(x, y) {
    // this.nodes[0].updateRelative(true, true); // no es necesario aquí
  ...
  draw(ctx) {
    ...
    // pequeñas “espinas” laterales para parecer huesos/ramas
    ctx.lineWidth = 1;
